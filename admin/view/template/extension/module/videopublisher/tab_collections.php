...
$(document).ready(function(){
	$.ajax({
		url: "index.php?route=<?php echo $modulePath; ?>/getCollections&token=<?php echo $token; ?>&page=1",
		type: 'get',
		dataType: 'html',
	...
	$('#openCollectionForm').on('click', function() {
		$.ajax({
			url: "<?php echo html_entity_decode($url->link($modulePath.'/addCollection', 'token=' . $token, 'SSL')); ?>",
			dataType: 'html',
...
function removeCollection(collection_id) {   
	...
	if (r==true) {
		$.ajax({
			url: 'index.php?route=<?php echo $modulePath; ?>/deleteCollection&token=<?php echo $token; ?>',
			type: 'post',
